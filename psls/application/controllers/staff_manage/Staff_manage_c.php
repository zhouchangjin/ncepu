...
class Staff_manage_c extends MY_Controller {
    ...
    {
    	$this->queryStaff();
    	$id = $this->data['user']->getId();
    	$param['id'] = $id;
    	$current_user = $this->StaffControl->getStaffList($param);
    	$param['department_id'] = $current_user[0]['department_id'];

    	$this->data['department']=$this->user_model->getQuery("select department_name,department_id from department_info"." where department_id=".$param['department_id']);

    	$this->data['current_department_name'] = $this->data['department'][0]['department_name'];
    	$this->data['current_department_id'] = $this->data['department'][0]['department_id'];

    	$this->ci_smarty->view('staff_manage/staff_query_v',$this->data);
	 ...
	 public function getRole()
	{
	 ...
	 public function addUser(){
	  ...
	  public function monitor(){
	 	$this->ci_smarty->view('staff_manage/monitor_v',$this->data);
	 }
	  public function signature(){
	 	$this->ci_smarty->view('staff_manage/signature_v',$this->data);
	 }
	 public function test1(){
	 	$this->ci_smarty->view('test1',$this->data);
	 }
 public function test2(){
	 	$this->ci_smarty->view('test2',$this->data);
	 }
	  public function test3(){
	 	$this->ci_smarty->view('test3',$this->data);
	 }
	  public function test4(){
	 	$this->ci_smarty->view('test4',$this->data);
	 }
	 public function administrator(){
    	...
    	$current_user = $this->StaffControl->getStaffList($param);
    	$param['department_id'] = $current_user[0]['department_id'];
	    $param['role_id'] = $current_user[0]['role_id'];
    	$this->data['department']=$this->user_model->getQuery("select department_name,department_id from department_info"." where department_id=".$param['department_id']);
        $this->data['role']=$this->user_model->getQuery("select role_name,role_id from role_info"." where role_id=".$param['role_id']);
    	$this->data['current_department_name'] = $this->data['department'][0]['department_name'];
    	$this->data['current_department_id'] = $this->data['department'][0]['department_id'];
    	$this->data['current_role_id'] = $this->data['role'][0]['role_id'];
	  ...
	  public function company(){
    	...
    	$this->data['current_department_name'] = $this->data['department'][0]['department_name'];
    	$this->data['current_department_id'] = $this->data['department'][0]['department_id'];
	 	$this->ci_smarty->view('staff_manage/company_query_v',$this->data);
	 }
	   public function station(){
    	...
    	$this->data['current_department_name'] = $this->data['department'][0]['department_name'];
    	$this->data['current_department_id'] = $this->data['department'][0]['department_id'];
	 	$this->ci_smarty->view('staff_manage/station_query_v',$this->data);
	 }
	 //璇ュ嚱鏁帮紝鐢ㄤ簬鏂板鍛樺伐
...
//echo $condition;
    	...
    	$sql="select * from tt_user NATURAL JOIN department_info NATURAL JOIN role_info ".$condition." ORDER BY role_info.status desc limit ".$page_start.",".$per_page;
 // echo $sql;die();
    	$this->data['userList']   = $this->user_model->getQuery($sql);
    	//$param                   = array();
		//$like                    = array();
    	//$this->data['userList']   = $this->user_model->get_user_list($param,'*',0,0,'','',$like);
